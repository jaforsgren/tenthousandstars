...
// Instantiates NarrativePanel the same way GameController does.
public partial class NarrativeDebugRoot : Control
{
	...
	public override void _Ready()
	{
		...
		{
			var captured = node;
			var btn = new Button { Text = captured };
			btn.Pressed += () => { _nodeInput.Text = captured; PlayNode(captured); };
	...
	private void OnPlayPressed()
	{
		try
		{
			PlayNode(_nodeInput.Text.Trim());
		}
		catch (Exception ex)
		{
			SetStatus(ex.Message, error: true);
			GD.PushError($"[NarrativeDebugRoot] {ex}");
		}
	}

	private void PlayNode(string nodeName)
	{
		...
		var panelScene = GD.Load<PackedScene>(NarrativePanelPath);
		if (panelScene == null)
		{
			SetStatus($"Could not load {NarrativePanelPath}", error: true);
			return;
		}

		var panel = panelScene.Instantiate<NarrativePanel>();
		AddChild(panel);
